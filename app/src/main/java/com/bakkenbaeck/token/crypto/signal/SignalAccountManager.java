...
/* package */ class SignalAccountManager extends SignalServiceAccountManager {
    ...
    /* package */ SignalAccountManager(final TrustStore trustStore) {
    ...
    private SignalAccountManager(final String url, TrustStore trustStore, String user, String password, String userAgent) {
    ...
    /* package */ void registerKeys(
        ...
        final PreKeyStateWithTimestamp payload = new PreKeyStateWithTimestamp(entities, lastResortEntity, signedPreKeyEntity, identityKey, timestamp);
        final String payloadForSigning = JsonUtil.toJson(payload);
        final String signature = BaseApplication.get().getUserManager().signPayload(payloadForSigning);
